...
import { getEvaluationQueue } from "#/services/evaluation-service";
...
export async function GET(_request: Request, { params }: { params: Promise<{ assignmentId: string }> }) {
  ...
  const pairs = await getEvaluationQueue(assignmentId, user.id);
  return NextResponse.json({ assignmentId, pairs });
}
